...
class cncfmUploader_svg2laser {
    ...
    getOptions = function () {
        ...
        $("#svg2laser tr.pass-settings").each(function (k) {
            ...
            var feedrate = $(this)
                .find(".pass-setting[data-setting=feedrate]")
                .val();
    ...
    init = function () {
        ...
        if (me.config.raster.thresholdDefault) {
            $("#svg2laser-raster-threshold").val(
                me.config.raster.thresholdDefault
            );
        }

        if (me.config.raster.powerMinDefault) {
            $("#svg2laser-raster-minpower").val(
                me.config.raster.powerMinDefault
            );
        }
