    ...
    for j in range(len(csp)):
        for i in range(1, len(csp[j])):
            d = csp_seg_to_point_distance(
                csp[j][i - 1], csp[j][i], p, sample_points=5)
    ...
    for k in range(sample_points):
        ...
        while i == 0 or abs(f) > 0.000001 and i < 20:
            ...
            df = (6 * ax * t + 2 * bx) * (ax * t3 + bx * t2 + cx * t + dx) + (3 * ax * t2 + 2 * bx * t + cx) ** 2 + \
                (6 * ay * t + 2 * by) * (ay * t3 + by * t2 + cy *
                                         t + dy) + (3 * ay * t2 + 2 * by * t + cy) ** 2
            if df != 0:
                t = t - f / df
            else:
                break
...
def csp_seg_to_csp_seg_distance(sp1, sp2, sp3, sp4, dist_bounds=(0, 1e100), sample_points=5, tolerance=.01):
    ...
    for k in range(sample_points):
        for j in range(sample_points):
            ...
            F2 = [[0, 0], [0, 0]]
            F = 1e100
            x = ax1 * t13 + bx1 * t12 + cx1 * t1 + dx1 - \
                (ax2 * t23 + bx2 * t22 + cx2 * t2 + dx2)
            y = ay1 * t13 + by1 * t12 + cy1 * t1 + dy1 - \
...
def csp_curvature_at_t(sp1, sp2, t, depth=3):
    ax, ay, bx, by, cx, cy, dx, dy = bezierparameterize(
...
def csp_from_arc(start, end, center, r, slope_st):
    ...
    alpha = (atan2(end[0] - center[0], end[1] - center[1]) -
             atan2(start[0] - center[0], start[1] - center[1])) % TAU
...
def csp_segment_convex_hull(sp1, sp2):
    ...
    raise ValueError(
        "csp_segment_convex_hull happened which is something that shouldn't happen!")
...
def draw_pointer(x, color="#f00", figure="cross", group=None, comment="", fill=None, width=.1, size=10., text=None, font_size=None, pointer_type=None, attrib=None):
    ...
    if figure == "line":
        ...
        attrib.update({"d": "M {},{} L {}".format(
            x[0], x[1], s), "style": "fill:none;stroke:{};stroke-width:{:f};".format(color, width), "comment": str(comment)})
    elif figure == "arrow":
        if fill is None:
            fill = "#12b3ff"
        fill_opacity = "0.8"
        d = "m {},{} ".format(x[0], x[1]) + re.sub("([0-9\\-.e]+)", (lambda match: str(float(match.group(1)) * size * 2.)),
        ...
        attrib.update({"d": d, "style": "fill:{};stroke:none;fill-opacity:{};".format(
            fill, fill_opacity), "comment": str(comment)})
    else:
        attrib.update({"d": "m {},{} l {:f},{:f} {:f},{:f} {:f},{:f} {:f},{:f} , {:f},{:f}".format(
            x[0], x[1], size, size, -2 * size, -2 * size, size, size, size, -size, -2 * size, 2 * size), "style": "fill:none;stroke:{};stroke-width:{:f};".format(color, width), "comment": str(comment)})
...
def csp_offset(csp, r):
    ...
    for s in joined_result[:]:
        dist = csp_to_point_distance(
            original_csp, s[int(len(s) / 2)][1], dist_bounds=[r1, r2])
        if not r1 < dist[0] < r2:
            joined_result.remove(s)
            if options.offset_draw_clippend_path:
...
class Postprocessor(object):
    ...
    def parse_command(self, command):
        ...
        if function in self.functions:
            print_("Postprocessor: executing function {}({})".format(
                function, parameters))
    ...
    def remap(self, parameters, case_sensitive=False):
        ...
        for i in range(len(pattern)):
            if case_sensitive:
                self.gcode = ireplace(
                    self.gcode, pattern[i], ":#:#:remap_pattern{}:#:#:".format(i))
            else:
                self.gcode = self.gcode.replace(
                    pattern[i], ":#:#:remap_pattern{}:#:#:".format(i))

        for i in range(len(remap)):
            self.gcode = self.gcode.replace(
                ":#:#:remap_pattern{}:#:#:".format(i), remap[i])
            ...
            if plane not in warned:
                ...
                if r:
                    ...
                    if plane == "g18" and scale[0] != scale[2]:
                        self.error(
                            "Post-processor: Scale factors for X and Z axis are not the same. G02 and G03 codes will be corrupted.")
                    if plane == "g19" and scale[1] != scale[2]:
                        self.error(
                            "Post-processor: Scale factors for Y and Z axis are not the same. G02 and G03 codes will be corrupted.")
                    warned += [plane]
            # Transform
                ...
                if move[i] != 0 or scale[i] != 1:
                    for a in axis[i]:
                        r = re.search(
                            r"(?i)(" + a + r")\s*(-?)\s*(\d*\.?\d*)", s)
                        if r and r.group(3) != "":
                            s = re.sub(r"(?i)(" + a + r")\s*(-?)\s*(\d*\.?\d*)", r"\1 {:f}".format(float(
                                r.group(2) + r.group(3)) * scale[i] + (move[i] if a not in ["i", "j", "k"] else 0)), s)
            # scale radius R
            if r_scale != 1:
                r = re.search(r"(?i)(r)\s*(-?\s*(\d*\.?\d*))", s)
                if r and r.group(3) != "":
                    try:
                        s = re.sub(r"(?i)(r)\s*(-?)\s*(\d*\.?\d*)",
                                   r"\1 {:f}".format(float(r.group(2) + r.group(3)) * r_scale), s)
                    except:
                        pass
    ...
    def parameterize(self, parameters):
        ...
        if len(planes) > 1:  # have G02 and G03 in several planes scale_x = scale_y = scale_z required
            ...
            gcode += "#10 = 1 ({} Scale factor)\n".format({"g17": "XY",
                                                           "g18": "XZ", "g19": "YZ"}[planes[0]])
            gcode += "#11 = 1 ({} Scale factor)\n".format({"g17": "Z",
                                                           "g18": "Y", "g19": "X"}[planes[0]])
            scale = {"x": "#10", "i": "#10", "y": "#10",
                     "j": "#10", "z": "#10", "k": "#10", "r": "#10"}
            if "g17" in planes:
                scale["z"] = "#11"
...
class Gcodetools(inkex.EffectExtension):
    ...
    def export_gcode(self, gcode, no_headers=False):
        ...
        with open(os.path.join(self.options.directory, self.options.file), "w") as f:
            ...
            f.write(self.options.gcodeHeader)
            f.write(gcode)
            f.write(self.options.gcodeFooter)
    ...
    def tab_plasma_prepare_path(self):
        ...
        if self.options.in_out_path or self.options.plasma_prepare_corners:
            ...
            if self.options.in_out_path_type == "Round" and self.options.in_out_path_len > self.options.in_out_path_radius * 3 / 2 * math.pi:
                self.error(
                    "In-out len is to big for in-out radius will cropp it to be r*3/2*pi!")
                ...
                if layer in self.selected_paths:
                    ...
                    for path in self.selected_paths[layer]:
                            ...
                            if self.options.in_out_path:
                                ...
                                if point_to_point_d2(subpath[0][1], subpath[-1][1]) < 1.e-10:
                                    ...
                                    for p in self.in_out_reference_points:
                                        d1 = csp_to_point_distance(
                                            [subpath], p, dist_bounds=[0, max_dist])
                                        if d1[0] < d[0]:
                                            d = d1[:]
                                            p_ = p
                                    if d[0] < max_dist ** 2:
                                        ...
                                        for j in range(len(subpath)):
                                            ...
                                            if point_to_point_d2(sp2[1], p_) < max_dist ** 2:
                                                s1 = csp_normalized_slope(
                                                    sp1, sp2, 1.)
                                                s2 = csp_normalized_slope(
                                                    sp2, sp3, 0.)
                                                max_cross = max(
                        ...
                        if self.options.in_out_path_replace_original_path:
                            path.path = CubicSuperPath(
                                self.apply_transforms(path, res, True))
                        else:
                            draw_csp(res, width=1,
                                     style=MARKER_STYLE["in_out_path_style"])

    def add_arguments(self, pars):
        add_argument = pars.add_argument
        add_argument("-d", "--directory", default="/home/",
                     help="Directory for gcode file")
        add_argument("-f", "--filename", dest="file",
                     default="-1.0", help="File name")
        add_argument("--add-numeric-suffix-to-filename", type=inkex.Boolean,
                     default=True, help="Add numeric suffix to filename")
        add_argument("--Zscale", type=float,
                     default="1.0", help="Scale factor Z")
        add_argument("--Zoffset", type=float,
                     default="0.0", help="Offset along Z")
        add_argument("-s", "--Zsafe", type=float, default="0.5",
                     help="Z above all obstacles")
        add_argument("-z", "--Zsurface", type=float,
                     default="0.0", help="Z of the surface")
        add_argument("-c", "--Zdepth", type=float,
                     default="-0.125", help="Z depth of cut")
        add_argument("--Zstep", type=float, default="-0.125",
                     help="Z step of cutting")
        add_argument("-p", "--feed", type=float, default="4.0",
                     help="Feed rate in unit/min")

        add_argument("--biarc-tolerance", type=float, default="1",
                     help="Tolerance used when calculating biarc interpolation.")
        add_argument("--biarc-max-split-depth", type=int, default="4",
                     help="Defines maximum depth of splitting while approximating using biarcs.")
        add_argument("--path-to-gcode-order", default="path by path",
                     help="Defines cutting order path by path or layer by layer.")
        add_argument("--path-to-gcode-depth-function",
                     default="zd", help="Path to gcode depth function.")
        add_argument("--path-to-gcode-sort-paths", type=inkex.Boolean,
                     default=True, help="Sort paths to reduce rapid distance.")
        add_argument("--comment-gcode", default="", help="Comment Gcode")
        add_argument("--comment-gcode-from-properties", type=inkex.Boolean,
                     default=False, help="Get additional comments from Object Properties")

        add_argument("--tool-diameter", type=float, default="3",
                     help="Tool diameter used for area cutting")
        add_argument("--max-area-curves", type=int, default="100",
                     help="Maximum area curves for each area")
        add_argument("--area-inkscape-radius", type=float, default="0",
                     help="Area curves overlapping (depends on tool diameter [0, 0.9])")
        add_argument("--area-tool-overlap", type=float, default="-10",
                     help="Radius for preparing curves using inkscape")
        add_argument("--unit", default="G21 (All units in mm)", help="Units")
        add_argument("--active-tab", type=self.arg_method('tab'),
                     default=self.tab_help, help="Defines which tab is active")

        add_argument("--area-fill-angle", type=float, default="0",
                     help="Fill area with lines heading this angle")
        add_argument("--area-fill-shift", type=float, default="0",
                     help="Shift the lines by tool d * shift")
        add_argument("--area-fill-method", default="zig-zag",
                     help="Filling method either zig-zag or spiral")

        add_argument("--area-find-artefacts-diameter", type=float,
                     default="1", help="Artefacts seeking radius")
        add_argument("--area-find-artefacts-action",
                     default="mark with an arrow", help="Artefacts action type")

        add_argument("--auto_select_paths", type=inkex.Boolean,
                     default=True, help="Select all paths if nothing is selected.")

        add_argument("--loft-distances", default="10",
                     help="Distances between paths.")
        add_argument("--loft-direction", default="crosswise",
                     help="Direction of loft's interpolation.")
        add_argument("--loft-interpolation-degree", type=float, default="2",
                     help="Which interpolation use to loft the paths smooth interpolation or staright.")

        add_argument("--min-arc-radius", type=float, default=".1",
                     help="All arc having radius less than minimum will be considered as straight line")

        add_argument("--engraving-sharp-angle-tollerance", type=float, default="150",
                     help="All angles thar are less than engraving-sharp-angle-tollerance will be thought sharp")
        add_argument("--engraving-max-dist", type=float, default="10",
                     help="Distance from original path where engraving is not needed (usually it's cutting tool diameter)")
        add_argument("--engraving-newton-iterations", type=int, default="4",
                     help="Number of sample points used to calculate distance")
        add_argument("--engraving-draw-calculation-paths", type=inkex.Boolean,
                     default=False, help="Draw additional graphics to debug engraving path")
        add_argument("--engraving-cutter-shape-function", default="w",
                     help="Cutter shape function z(w). Ex. cone: w. ")

        add_argument("--lathe-width", type=float,
                     default=10., help="Lathe width")
        add_argument("--lathe-fine-cut-width", type=float,
                     default=1., help="Fine cut width")
        add_argument("--lathe-fine-cut-count", type=int,
                     default=1., help="Fine cut count")
        add_argument("--lathe-create-fine-cut-using",
                     default="Move path", help="Create fine cut using")
        add_argument("--lathe-x-axis-remap", default="X",
                     help="Lathe X axis remap")
        add_argument("--lathe-z-axis-remap", default="Z",
                     help="Lathe Z axis remap")

        add_argument("--lathe-rectangular-cutter-width", type=float,
                     default="4", help="Rectangular cutter width")

        add_argument("--create-log", type=inkex.Boolean,
                     dest="log_create_log", default=False, help="Create log files")
        add_argument("--log-filename", default='', help="Create log files")

        add_argument("--orientation-points-count", default="2",
                     help="Orientation points count")
        add_argument("--tools-library-type",
                     default='cylinder cutter', help="Create tools definition")

        add_argument("--dxfpoints-action", default='replace',
                     help="dxfpoint sign toggle")

        add_argument("--help-language", default='http://www.cnc-club.ru/forum/viewtopic.php?f=33&t=35',
                     help="Open help page in webbrowser.")

        add_argument("--offset-radius", type=float,
                     default=10., help="Offset radius")
        add_argument("--offset-step", type=float,
                     default=10., help="Offset step")
        add_argument("--offset-draw-clippend-path", type=inkex.Boolean,
                     default=False, help="Draw clipped path")
        add_argument("--offset-just-get-distance", type=inkex.Boolean,
                     default=False, help="Don't do offset just get distance")

        add_argument("--postprocessor", default='',
                     help="Postprocessor command.")
        add_argument("--postprocessor-custom", default='',
                     help="Postprocessor custom command.")

        add_argument("--graffiti-max-seg-length", type=float,
                     default=1., help="Graffiti maximum segment length.")
        add_argument("--graffiti-min-radius", type=float,
                     default=10., help="Graffiti minimal connector's radius.")
        add_argument("--graffiti-start-pos", default="(0;0)",
                     help="Graffiti Start position (x;y).")
        add_argument("--graffiti-create-linearization-preview", type=inkex.Boolean,
                     default=True, help="Graffiti create linearization preview.")
        add_argument("--graffiti-create-preview", type=inkex.Boolean,
                     default=True, help="Graffiti create preview.")
        add_argument("--graffiti-preview-size", type=int,
                     default=800, help="Graffiti preview's size.")
        add_argument("--graffiti-preview-emmit", type=int,
                     default=800, help="Preview's paint emmit (pts/s).")

        add_argument("--in-out-path", type=inkex.Boolean,
                     default=True, help="Create in-out paths")
        add_argument("--in-out-path-do-not-add-reference-point", type=inkex.Boolean,
                     default=False, help="Just add reference in-out point")
        add_argument("--in-out-path-point-max-dist", type=float,
                     default=10., help="In-out path max distance to reference point")
        add_argument("--in-out-path-type", default="Round",
                     help="In-out path type")
        add_argument("--in-out-path-len", type=float,
                     default=10., help="In-out path length")
        add_argument("--in-out-path-replace-original-path",
                     type=inkex.Boolean, default=False, help="Replace original path")
        add_argument("--in-out-path-radius", type=float,
                     default=10., help="In-out path radius for round path")

        add_argument("--plasma-prepare-corners", type=inkex.Boolean,
                     default=True, help="Prepare corners")
        add_argument("--plasma-prepare-corners-distance", type=float,
                     default=10., help="Stepout distance for corners")
        add_argument("--plasma-prepare-corners-tolerance", type=float,
                     default=10., help="Maximum angle for corner (0-180 deg)")

        # cncfm
        add_argument("--laserpass", nargs="+", type=list, action="append",
                     dest="laserPass", help="#HEXCOLOR:FEEDRATE:POWER")
        add_argument("--gcode-header", dest="gcodeHeader",
                     default="", help="G-code header")
        add_argument("--gcode-footer", dest="gcodeFooter",
                     default="", help="G-code footer")
        add_argument("--gcode-on", dest="gcodeOn",
                     default="M3\n", help="G-code pre-path")
        add_argument("--gcode-off", dest="gcodeOff",
                     default="M5\n", help="G-code post-path")
        add_argument("--gcode-power", dest="gcodePower",
                     default="S{power}\n", help="G-code laser power")
    ...
    def draw_curve(self, curve, layer, group=None, style=MARKER_STYLE["biarc_style"]):
        ...
        if group is None:
            group = self.layers[min(1, len(self.layers) - 1)
                                ].add(Group(gcodetools="Preview group"))
        ...
        for sk in curve:
            ...
            si[2] = self.transform(si[2], layer, True) if type(
                si[2]) == type([]) and len(si[2]) == 2 else si[2]
    ...
    def check_dir(self):
        ...
        if os.path.isdir(self.options.directory):
            ...
            self.error(
                "Directory does not exist! Please specify existing directory at Preferences tab!", "error")
            return False
        ...
        try:
            ...
            self.error("Can not write to specified file!\n{}".format(
                os.path.join(self.options.directory, self.options.file)), "error")
            return False
        return True
    ...
    def generate_gcode(self, curve, layer, depth):
        ...
        def c(c):
            ...
            s = [" X", " Y", " Z", " I", " J", " K"]
            s1 = ["", "", "", "", "", ""]
            m = [1, 1, self.options.Zscale * Zauto_scale,
                 1, 1, self.options.Zscale * Zauto_scale]
        ...
        def calculate_angle(a, current_a):
            return min(
                [abs(a - current_a % TAU + TAU), a +
                 current_a - current_a % TAU + TAU],
                [abs(a - current_a % TAU - TAU), a +
                 current_a - current_a % TAU - TAU],
        ...
        current_a = 0
        go_to_safe_distance = ""  # cncfm "G00" + c([None, None, zs]) + "\n"
            ...
            if s[1] == 'move':
                ...
                if (r[0] ** 2 + r[1] ** 2) > self.options.min_arc_radius ** 2:
                    ...
                    if abs(r1.mag() - r2.mag()) < 0.001:
                        ...
                        r = (r1.mag() + r2.mag()) / 2
                        g += ("G02" if s[3] < 0 else "G03") + c(si[0] + [s[5]
                                                                         [1] + depth]) + " R{:f}".format(r) + feed + axis4 + "\n"
                    lg = 'G02'
                else:
                    if tool['4th axis meaning'] == "tangent knife":
    ...
    def get_transforms(self, g):
        ...
        while g != root:
            if 'transform' in g.keys():
                ...
                trans = (Transform(t) * Transform(trans)
                         ).matrix if trans != [] else t
            ...
            if not reverse:
                ...
                for comp in csp:
                    for ctl in comp:
                        for pt in ctl:
                            pt[0], pt[1] = Transform(
                                self.reverse_transform(trans)).apply_to_point(pt)
        return csp
    ...
    def transform(self, source_point, layer, reverse=False):
        if layer not in self.transform_matrix:
            ...
            if self.layers[i] not in self.orientation_points:
                self.error(
                    f"Orientation points for '{layer.label}' layer have not been found! Please add orientation points using Orientation tab!", "error")
                ...
                if len(self.orientation_points[orientation_layer]) > 1:
                    self.error(
                        f"There are more than one orientation point groups in '{orientation_layer.label}' layer")
                ...
                if len(points) == 3:
                    print_(
                        "Layer '{orientation_layer.label}' Orientation points: ")
                    for point in points:
                        print_(point)
                    ...
                    if numpy.linalg.det(matrix) != 0:
                        ...
                        self.error(
                            "Orientation points are wrong! (if there are two orientation points they should not be the same. If there are three orientation points they should not be in a straight line.)", "error")
                else:
            ...
            if self.svg.getElementById(elem_id) is None:
                ...
                path = marker.add(PathElement(
                    d="m {0}4.588864,-1.687441 0.0,0.0 L {0}9.177728,0.0 "
                      ...
                      .format(polA, polB)))
                path.style = "fill:{};fill-rule:evenodd;stroke:none;".format(
                    fill)
    ...
    def get_info(self):
            ...
            for i in items:
                ...
                if isinstance(i, Layer):
                    ...
                    if points is not None:
                        ...
                        print_(
                            f"Found orientation points in '{layer.label}' layer: {points}")
                    else:
                        self.error(
                            f"Warning! Found bad orientation points in '{layer.label}' layer. Resulting Gcode could be corrupt!")
                ...
                # Need to recognise old files ver 1.6.04 and earlier
                    ...
                    if point:
                        ...
                        self.error(
                            f"Warning! Found bad graffiti reference point in '{layer.label}' layer. Resulting Gcode could be corrupt!")
                    ...
                    for j in items_:
                        if j.get("gcodetools") == "In-out reference point":
                            self.in_out_reference_points.append(
                                self.apply_transforms(j, j.path.to_superpath())[0][0][1])
        ...
        if len(self.layers) == 1:
            self.error(
                "Document has no layers! Add at least one layer using layers panel (Ctrl+Shift+L)", "error")
        root = self.document.getroot()
        ...
        for i in p:
            ...
            for node in i:
                if node.get('gcodetools') == "Gcodetools orientation point arrow":
                    csp = node.path.transform(
                        node.composed_transform()).to_superpath()
                    point[0] = csp[0][0][1]
                if node.get('gcodetools') == "Gcodetools orientation point text":
                    inkex.errormsg(node.get_text())
                    r = re.match(
                        r'(?i)\s*\(\s*(-?\s*\d*(?:,|\.)*\d*)\s*;\s*(-?\s*\d*(?:,|\.)*\d*)\s*;\s*(-?\s*\d*(?:,|\.)*\d*)\s*\)\s*', node.get_text())
        ...
        for node in g:
            if node.get('gcodetools') == "Gcodetools graffiti reference point arrow":
                point[0] = self.apply_transforms(
                    node, node.path.to_superpath())[0][0][1]
            ...
            if i.get("gcodetools") == "Gcodetools tool background":
                ...
                if key in self.default_tool.keys():
                    try:
                        ...
                        self.error("Warning! Tool's and default tool's parameter's ({}) types are not the same ( type('{}') != type('{}') ).".format(
                            key, value, self.default_tool[key]))
                else:
                    tool[key] = value
                    self.error(
                        "Warning! Tool has parameter that default tool has not ( '{}': '{}' ).".format(key, value))
        return tool
    ...
    def set_tool(self, layer):
        ...
        if self.layers[i] in self.tools:
            ...
            self.error(
                f"Can not find tool for '{layer.label}' layer! Please add one with Tools library tab!", "error")
    ...
    def tab_path_to_gcode(self):
        ...
        parent = list(self.selected_paths)[
            0] if self.selected_paths else self.layers[0]
        ...
        for layer in self.layers:
            if layer in paths:
                ...
                try:
                    ...
                    self.error(
                        "Bad depth function! Enter correct function at Path to Gcode tab!")
                ...
                for path in paths[layer]:
                    ...
                    if path.get("dxfpoint") == "1":
                        ...
                        y = tmp_curve[0][0][0][1]
                        print_(
                            "got dxfpoint (scaled) at ({:f},{:f})".format(x, y))
                        dxfpoints += [[x, y]]
                    else:
                ...
                if self.options.path_to_gcode_order == 'path by path':
                    # cncfm (this whole block)

                    # print(self.options.path_to_gcode_sort_paths)
                    ...
                    for key in keys:
                        ...
                        color = str(colors[curves[key][0][0]]).lower()
                        if color not in gcodes:
                            gcodes[color] = ""
                        gcodes[color] += gcode_comment_str(
                            "start path: {}".format(curves[key][0][0]))
                        if curves[key][0][2] != "()":
                            gcodes[color] += curves[key][0][2]  # add comment

                        for curve in curves[key][1]:
                            gcodes[color] += self.generate_gcode(
                                curve, layer, -0.001)

                        gcodes[color] += gcode_comment_str(
                            "end path: {}".format(curves[key][0][0]))

        passColors = []
        passnum = 0
        for p in self.options.laserPass:
            passnum += 1
            p = "".join(p[0]).split(":")
            template_vals = {
                "passnum": passnum,
            ...
            color = p[0]
            template_vals["gcodePower"] = self.options.gcodePower.format(
                **template_vals)
            gcode += "\n"
            comment = "pass = {passnum}, stroke = {color}, feed = {feed}, power = {power}".format(
                **template_vals)
            gcode += gcode_comment_str(comment)
            if color in gcodes:
                # gcode += self.options.gcodeOn.format(**template_vals)
                gcode += "F{feed}\n".format(**template_vals)
                gcode += self.options.gcodePower.format(**template_vals)
                gcode += gcodes[color]
                gcode += self.options.gcodeOff.format(**template_vals)
    ...
    def tab_dxfpoints(self):
        ...
        if self.selected_paths == {}:
            self.error(
                "Nothing is selected. Please select something to convert to drill point (dxfpoint) or clear point sign.")
        for layer in self.layers:
            if layer in self.selected_paths:
                for path in self.selected_paths[layer]:
                    if self.options.dxfpoints_action == 'replace':
                        ...
                        if r is not None:
                            ...
                            path.set(
                                "d", "m {} 2.9375,-6.343750000001 0.8125,1.90625 6.843748640396,-6.84374864039 0,0 0.6875,0.6875 -6.84375,6.84375 1.90625,0.812500000001 z".format(r.group(1)))
    ...
    def tab_area_artefacts(self):
        ...
        for layer in paths:
            for path in paths[layer]:
                ...
                remove = []
                for i in range(len(csp)):
                    subpath = [[point[:] for point in points]
                               for points in csp[i]]
                    ...
                    if (bounds[2] - bounds[0]) ** 2 + (bounds[3] - bounds[1]) ** 2 < self.options.area_find_artefacts_diameter ** 2:
                        if self.options.area_find_artefacts_action == "mark with an arrow":
                            arrow = Path('m {},{} 2.9375,-6.343750000001 0.8125,1.90625 6.843748640396,-6.84374864039 0,0 0.6875,0.6875 -6.84375,6.84375 1.90625,0.812500000001 z'.format(
                                subpath[0][1][0], subpath[0][1][1])).to_superpath()
        ...
        for layer in self.layers:
            if layer in self.selected_paths:
                ...
                if self.tools[layer][0]['diameter'] <= 0:
                    self.error(
                        f"Tool diameter must be > 0 but tool's diameter on '{layer.label}' layer is not!", "error")
                ...
                for path in self.selected_paths[layer]:
                    ...
                    if path.get('sodipodi:type') != "inkscape:offset":
                        print_("Path {} is not an offset. Preparation started.".format(
                            path.get("id")))
                        # Path is not offset. Preparation will be needed.
                        # Finding top most point in path (min y value)

                        min_x, min_y, min_i, min_j, min_t = csp_true_bounds(csp)[
                            1]
                        ...
                        if min_y != float("-inf"):
                            ...
                            if min_t in [0, 1]:
                                ...
                                subp = [[subp[j][1], subp[j][1], subp[j][2]]] + subp[j +
                                                                                     1:] + subp[:j] + [[subp[j][0], subp[j][1], subp[j][1]]]
                            else:
                                sp1, sp2, sp3 = csp_split(
                                    subp[j - 1], subp[j], min_t)
                                subp[-1][2], subp[0][0] = subp[-1][1], subp[0][1]
                                subp = [[sp2[1], sp2[1], sp2[2]]] + [sp3] + subp[j +
                                                                                 1:] + subp[:j - 1] + [sp1] + [[sp2[0], sp2[1], sp2[1]]]
                            csp = [subp] + csp
                            # reverse path if needed
                    ...
                    for i in range(self.options.max_area_curves):
                        radius = - tool_d * \
                            (i * (1 - self.options.area_tool_overlap) + 0.5) * sign
                        if abs(radius) > abs(r):
                            radius = -r

                        elem = area_group.add(PathElement(
                            style=str(MARKER_STYLE["biarc_style_i"]['area'])))
                        elem.set('sodipodi:type', 'inkscape:offset')
                        elem.set('inkscape:radius', radius)
                        elem.set('inkscape:original', d)
                        print_(("adding curve", area_group, d, str(
                            MARKER_STYLE["biarc_style_i"]['area'])))
                        if radius == -r:
                            break
        ...
        def draw_point(xy0, xy, w, t):
            ...
            if self.options.engraving_draw_calculation_paths:
                ...
                if w:
                    ...
                    s2 = "#{0:x}0000".format(
                        int(101 * (1.5 - math.sin(s + 0.5))))
                    style = "stroke:{}; stroke-opacity:1;stroke-width:{};fill:none".format(
                        s2, t/2)
                    right = gcode_3Dleft.add(PathElement(
                        style=style, gcodetools="Gcode G1R"))
                    right.path = "M {:f},{:f} L {:f},{:f}".format(
                        x0 - eye_dist, y0, x - eye_dist - 0.14 * w, y)
                    left = gcode_3Dright.add(PathElement(
                        style=style, gcodetools="Gcode G1L"))
        ...
        for layer in self.layers:
            if layer in self.selected_paths and layer in self.orientation_points:
                ...
                if re.search('w', shape):
                    ...
                    self.error("Tool '{}' has no shape. 45 degree cone assumed!".format(
                        self.tools[layer][0]['name']))

                    def toolshape(w): return w
                ...
                max_distuu = min(
                    self.tools[layer][0]['diameter'] / 2, self.options.engraving_max_dist)
                max_dist = max_distuu * orientation_scale
                print_("max_dist pixels", max_dist)

                engraving_group = self.selected_paths[layer][0].getparent().add(
                    Group())
                ...
                if self.options.engraving_draw_calculation_paths:
                    gcode_3Dleft = self.my3Dlayer.add(
                        Group(gcodetools="Gcode 3D L"))
                    gcode_3Dright = self.my3Dlayer.add(
                        Group(gcodetools="Gcode 3D R"))
                        ...
                        for csp in cspi:  # LT6a For each subpath...
                            ...
                            for i in range(0, len(csp)):  # LT for each point
                                ...
                                if abs(1 - math.hypot(nx1, ny1)) > 0.00001:
                                    ...
                                    self.error(
                                        "csp_normalised_normal error. See log.")
                            ...
                            print_(("engraving_draw_calculation_paths=",
                            ...
                            if self.options.engraving_draw_calculation_paths:
                                ...
                                elem = gcode_3Dleft.add(PathElement(
                                    style=style, gcodetools="G1L outline"))
                                elem.path = CubicSuperPath([cspl])
                                elem = gcode_3Dright.add(Pathelement(
                                    style=style, gcodetools="G1R outline"))
                                elem.path = CubicSuperPath([cspr])

                                for p in nlLT[-1]:  # For last sub-path
                                    if p[2]:
                                        elem = engraving_group.add(
                                            PathElement(gcodetools="Engraving normals"))
                                        ...
                                        elem = engraving_group.add(PathElement(
                        ...
                        for j in xrange(len(nlLT)):  # LT6b for each subpath
                                ...
                                for b in xrange(bits):  # divide line into bits
                                    ...
                                    if reflex:  # just after a reflex corner
                                        ...
                                        if w < lastw:  # need to adjust it
                                            draw_point(
                                                (x1, y1), (n0[0][0] + n0[1][0] * w, n0[0][1] + n0[1][1] * w), w, (lastw - w) / 2)
                                            save_point(
                                    ...
                                    if n1[2]:  # We're at a corner
                                        if n1[3] > 0:  # acute
                                            ...
                                            if w > lastw:
                                                draw_point(
                                                    (x1, y1), (x1 + nx * lastw, y1 + ny * lastw), w, (w - lastw) / 2)
                                                wmax = max(wmax, w)
                                                save_point(
                                                    (x1 + nx * w, y1 + ny * w), w, i, j, iimin, jjmin)
                                    # acute corner coming up
                                        ...
                                        if jjmin == j and iimin == i + 2:
                                            break
                                    draw_point(
                                        (x1, y1), (x1 + nx * w, y1 + ny * w), w, bitlen)
                                    save_point(
                                        (x1 + nx * w, y1 + ny * w), w, i, j, iimin, jjmin)
                            ...
                            if self.options.engraving_draw_calculation_paths:
                                ...
                                for i in xrange(len(cspm)):
                                    elem = engraving_group.add(
                                        PathElement.arc(cspm[i][1], wl[i]))
                                    elem.set('gcodetools',
                                             "Engraving calculation paths")
                                    elem.style = "fill:none;fill-opacity:0.46;stroke:#000000;stroke-width:0.1;"
                            cspe += [cspm]
                ...
                if cspe:
                    curve = self.parse_curve(
                        cspe, layer, we, toolshape)  # convert to lines
                    self.draw_curve(curve, layer, engraving_group)
                    gcode += self.generate_gcode(curve,
                                                 layer, self.options.Zsurface)

            # LT3 for layers loop ends here
        if gcode != '':
            self.header += "(Tool diameter should be at least " + \
                str(2 * wmax / orientation_scale) + unit + ")\n"
            self.header += "(Depth, as a function of radius w, must be " + \
                self.tools[layer][0]['shape'] + ")\n"
    ...
    def tab_orientation(self, layer=None):
        ...
        if layer is None:
            layer = self.svg.get_current_layer() if self.svg.get_current_layer(
            ) is not None else self.document.getroot()
        ...
        if self.options.orientation_points_count == "graffiti":
            ...
            draw_text(axis, graffiti_reference_points_count * 100 + 10, -10,
                      group=group, gcodetools_tag="Gcodetools graffiti reference point text")

        elif self.options.orientation_points_count == "in-out reference point":
            draw_pointer(group=self.svg.get_current_layer(), x=self.svg.namedview.center,
            ...
            for i in points:
                ...
                draw_text("({}; {}; {})".format(i[0], i[1], i[2]), (i[0] + 10), (-i[1] - 10 +
                          doc_height), group=grp, gcodetools_tag="Gcodetools orientation point text")
    ...
    def tab_tools_library(self, layer=None):
        ...
        if layer in self.tools:
            self.error(
                "Active layer already has a tool! Remove it or select another layer!", "error")
...
G01 Z1 (going to cutting z)\n""",
        ...
        bg = tools_group.add(PathElement(
    ...
    def check_tools_and_op(self):
        if len(self.svg.selected) <= 0:
            ...
            paths = self.selected_paths
        #    Set group
        parent = self.selected_paths.keys()[0] if len(
            self.selected_paths.keys()) > 0 else self.layers[0]
    ...
    def generate_lathe_gcode(self, subpath, layer, feed_type):
        ...
        self.draw_curve(
            c, layer, style=MARKER_STYLE["biarc_style_lathe_{}".format(feed_type)])

        # Just in case move to the start...
        gcode = ("G01 {} {:f} {} {:f}".format(
            x, c[0][4][0], z, c[0][4][1])) + feed + "\n"
        for s in c:
            if s[1] == 'line':
                gcode += ("G01 {} {:f} {} {:f}".format(x,
                          s[4][0], z, s[4][1])) + feed + "\n"
            elif s[1] == 'arc':
                r = [(s[2][0] - s[0][0]), (s[2][1] - s[0][1])]
                if (r[0] ** 2 + r[1] ** 2) > self.options.min_arc_radius ** 2:
                    ...
                    r2 = (P(s[4]) - P(s[2]))
                    if abs(r1.mag() - r2.mag()) < 0.001:
                        gcode += ("G02" if s[3] * flip_angle < 0 else "G03") + (" {} {:f} {} {:f} {} {:f} {} {:f}".format(
                            x, s[4][0], z, s[4][1], i_, (s[2][0] - s[0][0]), k_, (s[2][1] - s[0][1]))) + feed + "\n"
                    else:
                        r = (r1.mag() + r2.mag()) / 2
                        gcode += ("G02" if s[3] * flip_angle < 0 else "G03") + (" {} {:f} {} {:f}".format(
                            x, s[4][0], z, s[4][1])) + " R{:f}".format(r) + feed + "\n"
        return gcode
    ...
    def tab_lathe(self):
        ...
        if x not in ["X", "Y", "Z", "x", "y", "z"] or z not in ["X", "Y", "Z", "x", "y", "z"]:
            self.error(
                "Lathe X and Z axis remap should be 'X', 'Y' or 'Z'. Exiting...")
            return
        if x.lower() == z.lower():
            self.error(
                "Lathe X and Z axis remap should be the same. Exiting...")
            return
        ...
        for layer in self.layers:
            if layer in paths:
                ...
                if self.tool != self.tools[layer][0]:
                    ...
                    self.tool["passing feed"] = float(
                        self.tool["passing feed"] if "passing feed" in self.tool else self.tool["feed"])
                    self.tool["feed"] = float(self.tool["feed"])
                    self.tool["fine feed"] = float(
                        self.tool["fine feed"] if "fine feed" in self.tool else self.tool["feed"])
                    ...
                    for subpath in csp:
                        ...
                        if self.options.lathe_fine_cut_width > 0:
                            ...
                            if self.options.lathe_create_fine_cut_using == "Move path":
                                subpath = [[[i2[0], i2[1] + r]
                                            for i2 in i1] for i1 in subpath]
                            else:
                                # Close the path to make offset correct
                                bound = csp_simple_bound([subpath])
                                minx, miny, maxx, maxy = csp_true_bounds(
                                    [subpath])
                                offsetted_subpath = csp_subpath_line_to(subpath[:], [[subpath[-1][1][0], miny[1] - r * 10], [
                                                                        subpath[0][1][0], miny[1] - r * 10], [subpath[0][1][0], subpath[0][1][1]]])
                                left = subpath[-1][1][0]
                                right = subpath[0][1][0]
                                if left > right:
                                    left, right = right, left
                                offsetted_subpath = csp_offset(
                                    [offsetted_subpath], r if not csp_subpath_ccw(offsetted_subpath) else -r)
                                offsetted_subpath = csp_clip_by_line(
                                    offsetted_subpath, [left, 10], [left, 0])
                                offsetted_subpath = csp_clip_by_line(
                                    offsetted_subpath, [right, 0], [right, 10])
                                offsetted_subpath = csp_clip_by_line(
                                    offsetted_subpath, [0, miny[1] - r], [10, miny[1] - r])
                                # Join offsetted_subpath together
                                # Hope there won't be any circles
                                subpath = csp_join_subpaths(
                                    offsetted_subpath)[0]

                        # Create solid object from path and lathe_width
                        bound = csp_simple_bound([subpath])
                        top_start = [subpath[0][1][0], self.options.lathe_width +
                                     self.options.Zsafe + self.options.lathe_fine_cut_width]
                        top_end = [subpath[-1][1][0], self.options.lathe_width +
                                   self.options.Zsafe + self.options.lathe_fine_cut_width]

                        gcode += ("G01 {} {:f} F {:f} \n".format(z,
                                  top_start[1], self.tool["passing feed"]))
                        gcode += ("G01 {} {:f} {} {:f} F {:f} \n".format(x,
                                  top_start[0], z, top_start[1], self.tool["passing feed"]))

                        subpath = csp_concat_subpaths(csp_subpath_line_to(
                            [], [top_start, subpath[0][1]]), subpath)
                        subpath = csp_subpath_line_to(
                            subpath, [top_end, top_start])

                        width = max(0, self.options.lathe_width -
                                    max(0, bound[1]))
                        ...
                        for i in range(steps + 1):
                            ...
                            for j in range(1, len(subpath)):
                                sp1 = subpath[j - 1]
                                sp2 = subpath[j]
                                intersections += [[j, k] for k in csp_line_intersection(
                                    [bound[0] - 10, current_width], [bound[2] + 10, current_width], sp1, sp2)]
                                intersections += [[j, k] for k in csp_line_intersection(
                                    [bound[0] - 10, current_width + step], [bound[2] + 10, current_width + step], sp1, sp2)]
                            parts = csp_subpath_split_by_points(
                                subpath, intersections)
                            for part in parts:
                                minx, miny, maxx, maxy = csp_true_bounds([
                                                                         part])
                                y = (maxy[1] + miny[1]) / 2
                                if y > current_width + step:
                                    gcode += self.generate_lathe_gcode(
                                        part, layer, "passing feed")
                                elif current_width <= y <= current_width + step:
                                    gcode += self.generate_lathe_gcode(
                                        part, layer, "feed")
                                else:
                                    # full step cut
                                    part = csp_subpath_line_to(
                                        [], [part[0][1], part[-1][1]])
                                    gcode += self.generate_lathe_gcode(
                                        part, layer, "feed")

                        top_start = [fine_cut[0][1][0], self.options.lathe_width +
                                     self.options.Zsafe + self.options.lathe_fine_cut_width]
                        top_end = [fine_cut[-1][1][0], self.options.lathe_width +
                                   self.options.Zsafe + self.options.lathe_fine_cut_width]
                        gcode += "\n(Fine cutting start)\n(Calculating fine cut using {})\n".format(
                            self.options.lathe_create_fine_cut_using)
                        for i in range(int(self.options.lathe_fine_cut_count)):
                            width = self.options.lathe_fine_cut_width * \
                                (1 - float(i + 1) / self.options.lathe_fine_cut_count)
                            if width == 0:
                                current_pass = fine_cut
                            else:
                                if self.options.lathe_create_fine_cut_using == "Move path":
                                    current_pass = [[[i2[0], i2[1] + width]
                                                     for i2 in i1] for i1 in fine_cut]
                                else:
                                    minx, miny, maxx, maxy = csp_true_bounds(
                                        [fine_cut])
                                    offsetted_subpath = csp_subpath_line_to(fine_cut[:], [[fine_cut[-1][1][0], miny[1] - r * 10], [
                                                                            fine_cut[0][1][0], miny[1] - r * 10], [fine_cut[0][1][0], fine_cut[0][1][1]]])
                                    left = fine_cut[-1][1][0]
                                    right = fine_cut[0][1][0]
                                    if left > right:
                                        left, right = right, left
                                    offsetted_subpath = csp_offset(
                                        [offsetted_subpath], width if not csp_subpath_ccw(offsetted_subpath) else -width)
                                    offsetted_subpath = csp_clip_by_line(
                                        offsetted_subpath, [left, 10], [left, 0])
                                    offsetted_subpath = csp_clip_by_line(
                                        offsetted_subpath, [right, 0], [right, 10])
                                    offsetted_subpath = csp_clip_by_line(
                                        offsetted_subpath, [0, miny[1] - r], [10, miny[1] - r])
                                    current_pass = csp_join_subpaths(
                                        offsetted_subpath)[0]
            ...
            if layer in paths:
                ...
                for path in paths[layer]:
                    ...
                    for subpath in csp:
                        ...
                        for sp1, sp2 in zip(subpath[:], subpath[1:]):
                            ...
                            if a != 0 and a != math.pi:
                                ...
                                if not first_seg and new_subpath == []:
                                    new_subpath = [
                                        [[subpath[0][i][0] - width * o, subpath[0][i][1]] for i in range(3)]]

                                new_subpath = csp_concat_subpaths(
                                    new_subpath,
                                    [
                                        [[sp1[i][0] - width * o, sp1[i][1]]
                                         for i in range(3)],
                                        [[sp2[i][0] - width * o, sp2[i][1]]
                                         for i in range(3)]
                                    ]
                                )
                            first_seg = False
                        ...
                        if a == 0 or a == math.pi:
                            new_subpath += [[[subpath[-1][i][0] - width *
                                              o, subpath[-1][i][1]] for i in range(3)]]
    ...
    def tab_graffiti(self):
        ...
        def get_gcode_coordinates(point, layer):
            ...
            for ref_point in self.graffiti_reference_points[layer]:
                c = math.sqrt((point[0] - ref_point[0][0])
                              ** 2 + (point[1] - ref_point[0][1]) ** 2)
            ...
            for x in range(int(x1 - 1 - math.ceil(radius)), int(x1 + 1 + math.ceil(radius) + 1)):
                for y in range(int(y1 - 1 - math.ceil(radius)), int(y1 + 1 + math.ceil(radius) + 1)):
                    if x >= 0 and y >= 0 and y < len(self.graffiti_preview) and x * 4 < len(self.graffiti_preview[0]):
                        ...
                        a = float(a_) * (max(0, (1 - (d - radius)))
                                         if d > radius else 1) / 256
                        self.graffiti_preview[y][x * 4] = int(
                            r * a + (1 - a) * self.graffiti_preview[y][x * 4])
                        self.graffiti_preview[y][x * 4 + 1] = int(
                            g * a + (1 - a) * self.graffiti_preview[y][x * 4 + 1])
                        self.graffiti_preview[y][x * 4 + 2] = int(
                            g * b + (1 - a) * self.graffiti_preview[y][x * 4 + 2])
                        self.graffiti_preview[y][x * 4 + 3] = min(
                            255, int(self.graffiti_preview[y][x * 4 + 3] + a * 256))
        ...
        def draw_graffiti_segment(layer, start, end, feed, color=(0, 255, 0, 40), emmit=1000):
            ...
            l = math.sqrt(
                sum([(start[i] - end[i]) ** 2 for i in range(len(start))]))
            time_ = l / feed
            ...
            if d == 0:
                raise ValueError(
                    "Error! Reference points should not be the same!")
...
(Header end.)"""
            ...
            if layer in paths:
                ...
                if layer not in self.graffiti_reference_points:
                    ...
                    if reference_points is None:
                        self.error(
                            'There are no graffiti reference points for layer {}'.format(layer), "error")

                # Transform reference points
                for i in range(len(self.graffiti_reference_points[layer])):
                    self.graffiti_reference_points[layer][i][0] = self.transform(
                        self.graffiti_reference_points[layer][i][0], layer)
        ...
        if self.options.graffiti_create_preview:
            self.graffiti_preview = list([[255] * (4 * self.options.graffiti_preview_size)
                                         for _ in range(self.options.graffiti_preview_size)])
        ...
        for layer in self.layers:
            if layer in paths:
                ...
                r = re.match(
                    "\\s*\\(\\s*([0-9\\-,.]+)\\s*;\\s*([0-9\\-,.]+)\\s*\\)\\s*", self.options.graffiti_start_pos)
                if r:
                    start_point = [float(r.group(1)), float(r.group(2))]
                else:
                    start_point = [0., 0.]
                last_sp1 = [[start_point[0], start_point[1] - 10]
                            for _ in range(3)]
                ...
                while len(subpaths) > 0:
                    i = min([(point_to_point_d2(last_sp2[1], subpaths[i][0][1]), i)
                    ...
                    for sp1, sp2 in zip(subpath, subpath[1:]):
                        # TODO add coefficient into inx
                        if spl is not None and abs(cross(csp_normalized_slope(spl, sp1, 1.), csp_normalized_slope(sp1, sp2, 0.))) > 0.1:
                ...
                if not polylines:
                    continue
                polylines += [["connect1", [[polylines[-1][1][-1][1]
                                             for _ in range(3)], [start_point for _ in range(3)]]]]
                ...
                for i in range(len(polylines)):
                    ...
                    for sp1, sp2 in zip(polylines[i][1], polylines[i][1][1:]):
                        ...
                        if l > 0.00000001:
                            polyline += [sp1[1]]
                            parts = int(
                                math.ceil(l / self.options.graffiti_max_seg_length))
                            for j in range(1, parts):
                                polyline += [csp_at_length(sp1,
                                                           sp2, float(j) / parts)]
                    if l > 0.00000001:
                        polyline += [sp2[1]]
                    print_(i)
                ...
                for polyline_ in polylines:
                    ...
                    if self.options.graffiti_create_linearization_preview:
                        t += 1
                        csp = [[polyline[i], polyline[i], polyline[i]]
                               for i in range(len(polyline))]
                        draw_csp(self.transform_csp(
                            [csp], layer, reverse=True))
                    ...
                    last_real_pos, g = get_gcode_coordinates(
                        polyline[0], layer)
                    last_pos = polyline[0]
                    ...
                    for point in polyline:
                        ...
                        real_l = sum(
                            [(real_pos[i] - last_real_pos[i]) ** 2 for i in range(len(last_real_pos))])
                        l = (last_pos[0] - point[0]) ** 2 + \
                            (last_pos[1] - point[1]) ** 2
                        if l != 0:
                            feed = self.tool['feed'] * math.sqrt(real_l / l)
                            gcode += "G01 " + g + " F {:f}\n".format(feed)
                            if self.options.graffiti_create_preview:
                                draw_graffiti_segment(layer, real_pos, last_real_pos, feed, color=(0, 0, 255, 200) if polyline_[
                                                      0] == "draw" else (255, 0, 0, 200), emmit=self.options.graffiti_preview_emmit)
        ...
        if self.options.graffiti_create_preview:
            try:
                ...
                for layer in self.graffiti_reference_points:
                    for point in self.graffiti_reference_points[layer]:
                        x, y = graffiti_preview_transform(
                            point[0][0], point[0][1])
                        graffiti_preview_draw_point(
                            x, y, (0, 255, 0, 255), radius=5)

                import png
                writer = png.Writer(width=self.options.graffiti_preview_size, height=self.options.graffiti_preview_size, size=None, greyscale=False, alpha=True, bitdepth=8, palette=None,
                                    transparent=None, background=None, gamma=None, compression=None, interlace=False, bytes_per_sample=None, planes=None, colormap=None, maxval=None, chunk_limit=1048576)
            ...
            for path in self.selected_paths[layer]:
                ...
                while abs(offset) <= abs(self.options.offset_radius):
                    ...
                    if offset_:
                        ...
                        print_(
                            "------------Reached empty offset at radius {}".format(offset))
                        break
